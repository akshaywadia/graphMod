...
object Test {
  def main(args : Array[String]) {
    ...
    println("First run over at " + new Date())
    ...
    var i : Int = 0
    while (i < 10) {
      println("Stage " + i.toString + " Starting reset at " + new Date())
     ...
     // --  var modgr = gmod.resetGraph(postrun)
      ...
      println("\t\tUpdateEdge at " + new Date()) 

      var gup = gmod.updateEdge(ed, modgr) 

      println("\t\tInc run at " + new Date())
      --- */
      // --- postrun = gmod.run(gup)
      postrun = gmod.run(postrun)
      i += 1
    }
    println("\n")
